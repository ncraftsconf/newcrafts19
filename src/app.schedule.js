...
function generateEventsHtml (days) {
	...
	for (var i = 0; i < days.length; i++) {
		scheduleContent += '<div id="day' + i + '" class="tab-pane fade in">';

		const rooms = days[i].rooms;

		if (rooms.length > 1) {

			scheduleContent += '<ul class="nav nav-schedule">';
		...
		scheduleContent += '<div class="tab-content tab-content-schedule">';
		...
		for (var j = 0; j < rooms.length; j++) {
			...
			for (var k = 0; k < events.length; k++) {
				...
				if (events[k].description !== '' || events[k].speakerName !== '') {
					scheduleContent += '<a data-toggle="collapse" data-parent="#day' + i + '_room' + j + '_timeline" href="#day' + i + '_room' + j + '_timeline_time' + k + '" class="schedule-item-toggle collapsed">'
